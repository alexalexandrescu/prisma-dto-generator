...
import type { DTOFile, EnumInfo, FieldInfo, GeneratorConfig, ModelInfo } from './types'
...
export class DTOGenerator {
  ...
  private generateCreateDTO(model: ModelInfo): DTOFile {
    ...
    const properties = fields.map((field) => this.generateProperty(field, false)).join('\n\n  ')
...
`
  ...
  private generateReadDTO(model: ModelInfo): DTOFile {
    ...
    const imports = this.generateImports(fields)
    const properties = fields.map((field) => this.generateProperty(field, true)).join('\n\n  ')
...
`
    ...
    for (const domain of domains) {
      ...
      for (const [subfolder, modelNames] of subfolderGroups) {
        ...
        for (const modelName of modelNames) {
          subfolderExports.push(`export { Create${modelName}Dto } from './create-${this.toKebabCase(modelName)}.dto';`)
          subfolderExports.push(`export { Update${modelName}Dto } from './update-${this.toKebabCase(modelName)}.dto';`)
          subfolderExports.push(`export { ${modelName}Dto } from './read-${this.toKebabCase(modelName)}.dto';`)
        }
  ...
  private getFieldsForRead(model: ModelInfo): FieldInfo[] {
    ...
    return model.fields.filter((field) => !omitFields.includes(field.name) && !field.isRelation)
  }
  ...
  private generateImports(fields: FieldInfo[]): string {
    ...
    imports.add("import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';")
  ...
  private generateProperty(field: FieldInfo, isReadOnly: boolean): string {
    ...
    if (mapping.isArray) {
      swaggerOptions.push(`type: ${mapping.enumName || `'${mapping.swaggerType}'`}`)
    ...
    const swaggerDecorator = mapping.isOptional || mapping.isNullable ? 'ApiPropertyOptional' : 'ApiProperty'
    ...
    const propertyName = mapping.isOptional && !mapping.isNullable ? `${field.name}?` : field.name
