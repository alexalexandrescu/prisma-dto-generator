...
describe('Generator Index', () => {
	const mockGeneratorHandler = generatorHandler as jest.MockedFunction<
		typeof generatorHandler
	>;
	...
	describe('onGenerate', () => {
		it('should handle basic configuration', async () => {
			...
			if (onGenerate) {
				...
				expect(mockFs.mkdirSync).toHaveBeenCalledWith('../generated/dto', {
					recursive: true,
				});
			}
		});
		...
		it('should clean output directory when clean option is enabled', async () => {
			...
			if (onGenerate) {
				...
				expect(mockFs.existsSync).toHaveBeenCalledWith('../generated/dto');
				expect(mockFs.rmSync).toHaveBeenCalledWith('../generated/dto', {
					recursive: true,
					force: true,
				});
				expect(consoleSpy).toHaveBeenCalledWith(
					'Cleaning output directory: ../generated/dto',
				);
				expect(mockFs.mkdirSync).toHaveBeenCalledWith('../generated/dto', {
					recursive: true,
				});
		...
		it('should handle invalid JSON configuration gracefully', async () => {
			...
			if (onGenerate) {
				...
				expect(consoleSpy).toHaveBeenCalledWith(
					'Failed to parse omitFields config:',
					expect.any(Error),
				);
			}
