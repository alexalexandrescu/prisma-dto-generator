...
describe('Integration Tests', () => {
	describe('Full DMMF to DTOs workflow', () => {
		it('should process complete DMMF and generate all DTOs with flat structure', () => {
			...
			const userUpdateFile = files.find(
				(f) => f.fileName === 'update-user.dto.ts',
			);
			...
			const mediaUpdateFile = files.find(
				(f) => f.fileName === 'update-media-asset.dto.ts',
			);
			const mediaReadFile = files.find(
				(f) => f.fileName === 'read-media-asset.dto.ts',
			);
			...
			expect(mediaCreateFile?.content).toContain(
			...
			expect(barrelFile?.content).toContain(
				"export { CreateMediaAssetDto } from './create-media-asset.dto'",
			);
		});
		...
		it('should process complete DMMF and generate all DTOs with domain structure', () => {
			...
			const mediaBarrelFile = files.find(
				(f) => f.fileName === 'index.ts' && f.folderPath === 'media',
			);
			...
			const mediaSubfolderBarrelFile = files.find(
				(f) =>
					f.fileName === 'index.ts' && f.folderPath === 'media/media-asset',
			);
			...
			expect(userManagementBarrelFile?.content).toContain(
				"export * from './user'",
			);
			expect(mediaBarrelFile?.content).toContain(
		...
		it('should handle complex field types and relationships', () => {
			...
			expect(createFile?.content).toContain(
				'metadata: Record<string, unknown> | null',
			);
