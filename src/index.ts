...
generatorHandler({
	...
	onGenerate: async (options) => {
		...
		const config: GeneratorConfig = {
			...
			relations:
				(generator.config.relations as string as 'omit' | 'ids' | 'nested') ||
				'ids',
			dateStrategy:
				(generator.config.dateStrategy as string as 'iso-string' | 'date') ||
				'iso-string',
			jsonType:
				(generator.config.jsonType as string) || 'Record<string, unknown>',
			fileNaming:
				(generator.config.fileNaming as string as
					| 'kebab'
					| 'camel'
					| 'pascal') || 'kebab',
			heuristics: generator.config.heuristics === 'true',
			folderStructure:
				(generator.config.folderStructure as string as 'flat' | 'domain') ||
				'flat',
		...
		if (generator.config.readDtoInclude) {
			try {
				const readDtoIncludeValue = Array.isArray(
