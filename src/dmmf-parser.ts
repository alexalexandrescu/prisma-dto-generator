...
export class DMMFParser {
  parse(dmmf: DMMF.Document): { models: ModelInfo[]; enums: EnumInfo[] } {
    ...
    const models = dmmf.datamodel.models.map((model) => this.parseModel(model, enums))
  ...
  private parseField(field: DMMF.Field, enums: EnumInfo[]): FieldInfo {
    ...
    const isNullable = field.kind === 'scalar' && field.type !== 'Boolean' && !field.isRequired // Only scalar non-boolean optional fields are nullable
    ...
    return {
      ...
      relationType,
      enumName,
      enumValues: enumName ? enums.find((e) => e.name === enumName)?.values : undefined
    }
  }
}
